...
void ICACHE_FLASH_ATTR sniffer_wifi_promiscuous_rx(uint8 *buf, uint16 buf_len)
{
    ...
    printf("In %s, the buffer length is %d.\r\n", __func__, buf_len);

    if (buf_len == sizeof(struct sniffer_buf2)) {/* managment frame */
        printf("This is a management packet.\r\n");
        ...
        data = sniffer->buf;
        mngt_data_cb(data, data_len, &info);
    }  else if (buf_len == sizeof(struct RxControl)) {/* mimo, HT40, LDPC */
        printf("Warning: MIMO/HT40/LDPC packet, ignore it for now.\r\n");
    } else {
...
{
    data_cb = fn;
    printf("WiFi monitor callback registered.\r\n");
}

static void register_wlan_mgnt_monitor_cb(hal_wifi_module_t *m, monitor_data_cb_t fn)
{
    mngt_data_cb = fn;
    printf("WiFi management monitor callback registered.\r\n");
...
{
    if (wifi_send_pkt_freedom(buf, len, true) != 0) {
        printf("%s failed.\r\n");
        return -1;
    }
